        ...
        for v_id, state in vehicles_data.items():
            ...
            st.markdown(
                ...
                <div style="background-color: {bg_color}; padding: 10px; border-radius: 5px; color: black; margin-bottom: 10px;">
                    ...
                    Швидкість: {state.get('speed_buffer', [0])[-1]:.1f} км/год
                </div>
...
with col_map:
    ...
    if route_data:
        path_layer = pdk.Layer(
            ...
            get_path="path",
            get_color=[150, 150, 150, 150],  # Сіра лінія
        ...
        endpoint_layer = pdk.Layer(
            "ScatterplotLayer",
            data=endpoints, get_position="coord", get_fill_color="color",
            get_radius=50, radius_min_pixels=6,
        )
    ...
    if vehicles_data:
        ...
        for v_id, state in vehicles_data.items():
            ...
            color = [255, 50, 50, 255] if is_pred else [50, 255, 50, 255] # Червоний/Зелений
            v_list.append({
                "coord": [state["lon"], state["lat"]],
                "color": color, "tooltip": f"Авто: {v_id}"
            })
            
        vehicle_layer = pdk.Layer(
            "ScatterplotLayer",
            data=v_list, get_position="coord", get_fill_color="color",
            get_line_color=[0, 0, 0, 255], stroked=True,
            line_width_min_pixels=2, get_radius=60, radius_min_pixels=10,
        )
        layers.append(vehicle_layer)

    # 3. НАЛАШТУВАННЯ КАМЕРИ
    start_lat, start_lon = 50.462, 30.515
    if route_data: start_lat, start_lon = route_data["start"]["lat"], route_data["start"]["lon"]
    if vehicles_data:
